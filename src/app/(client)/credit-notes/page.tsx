...
export default function CreditNotesPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/invoices").then((r) => r.json()).then((res) => {
      ...
      setNotes(all.filter((i) => i.invoiceType === "credit_note"));
      setInvoices(all.filter((i) => i.invoiceType === "tax_invoice" || i.invoiceType === "bill_of_supply"));
  ...
  const handleDownloadExcel = async () => {
    ...
    const data = filtered.map((n) => ({
      "Credit Note #": n.invoiceNumber, "Date": n.date, "Customer": n.customer.name, "GSTIN": n.customer.gstin || "",
    ...
    XLSX.writeFile(wb, `Credit_Notes_${monthFilter || "All"}.xlsx`);
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Issue credit notes for returns, refunds, or price adjustments</p>
        </div>
        <Link href="/create-invoice?type=credit_note" className="flex items-center gap-2 bg-rose-600 text-white px-4 py-2 rounded-lg hover:bg-rose-700 text-sm font-medium">
          <FilePlus className="w-4 h-4" /> Create Credit Note
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white rounded-xl shadow-sm border p-4">
          <p className="text-xs text-gray-500 font-medium">Total Credit Notes</p>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search credit notes..." className="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500" />
        </div>
      ...
      {/* Table */}
      {loading ? (
        <div className="flex justify-center py-12"><div className="animate-spin w-8 h-8 border-4 border-rose-600 border-t-transparent rounded-full" /></div>
      ) : (
            ...
            <tbody>
              {filtered.map((n) => (
                <tr key={n.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-3 text-right font-medium text-rose-600">{formatCurrency(n.grandTotal)}</td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-right">
                    <Link href={`/invoice-view?id=${n.id}`} className="p-1.5 hover:bg-gray-100 rounded inline-block" title="View"><Eye className="w-4 h-4 text-indigo-500" /></Link>
                    <Link href={`/create-invoice?edit=${n.id}`} className="p-1.5 hover:bg-blue-50 rounded inline-block" title="Edit"><Edit2 className="w-4 h-4 text-blue-500" /></Link>
                    <button onClick={() => handleDelete(n.id)} className="p-1.5 hover:bg-gray-100 rounded" title="Delete"><Trash2 className="w-4 h-4 text-red-500" /></button>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && <tr><td colSpan={7} className="p-8 text-center text-gray-400">No credit notes found. Create one to issue refunds or adjust invoices.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
