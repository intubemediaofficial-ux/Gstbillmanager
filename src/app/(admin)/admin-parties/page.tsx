...
interface AdminParty extends Customer {
  ownerName: string;
  ownerEmail: string;
  ownerId: string;
}
...
export default function AdminPartiesPage() {
  ...
  const [editing, setEditing] = useState<AdminParty | null>(null);
  ...
  const handleDelete = async (p: AdminParty) => {
    if (!confirm(`Delete party "${p.name}"?`)) return;
    await fetch("/api/admin/parties", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "delete", userId: p.ownerId, id: p.id }),
    });
    fetchParties();
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">All Client Parties (Bill To)</h1>
      ...
      <div className="mb-4 relative max-w-md">
        ...
        <input
          ...
          placeholder="Search by party name, owner, or GSTIN..."
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="p-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${p.isGst ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}>
                      {p.isGst ? "GST" : "Non-GST"}
                    </span>
                  </td>
                  <td className="p-3 font-mono text-xs text-gray-500">{p.gstin || "-"}</td>
                  <td className="p-3">
                    <div className="font-medium">{p.ownerName}</div>
                    <div className="text-xs text-gray-400">{p.ownerEmail}</div>
                  </td>
                  <td className="p-3 text-gray-500">{p.state || "-"}</td>
                  <td className="p-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(p)} className="p-1.5 hover:bg-blue-50 rounded text-blue-600" title="Edit">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(p)} className="p-1.5 hover:bg-red-50 rounded text-red-600" title="Delete">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="p-8 text-center text-gray-400">No parties found</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold">Edit Party — {editing.ownerName}</h2>
