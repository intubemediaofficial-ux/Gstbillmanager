...
import { FileText, Trash2, Calendar, Building2, User, Search, Filter, Users, Mail } from "lucide-react";
...
export default function AdminDocumentsPage() {
  ...
  const handleDelete = async (doc: SavedDoc) => {
    if (!confirm(`Delete "${doc.title}" from ${doc.userName}'s history?`)) return;
    await fetch("/api/documents", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: doc.id, userId: doc.userId }),
    });
    setDocs((prev) => prev.filter((d) => d.id !== doc.id));
  };

  const types = [...new Set(docs.map((d) => d.type))].sort();
  const users = [...new Set(docs.map((d) => d.userName))].sort();
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800">All Client Documents</h1>
        <p className="text-sm text-gray-500 mt-1">View all documents generated by all clients</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          <p className="text-xs text-gray-500">Document Types Used</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search by title, client, firm..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none" />
        </div>
        ...
        <div className="relative">
          ...
          <select value={userFilter} onChange={(e) => setUserFilter(e.target.value)} className="pl-10 pr-8 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none appearance-none bg-white">
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">Documents generated by clients will appear here</p>
        </div>
      ) : (
                  ...
                  <tr key={doc.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-gray-800 truncate max-w-[180px]">{doc.title}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-gray-600 hidden sm:table-cell">{doc.recipientName || "-"}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => handleDelete(doc)} className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition" title="Delete">
